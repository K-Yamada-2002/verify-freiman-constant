...
検証は\texttt{verify\_case\_family\_v\_cover}函数によって行われている．

この函数は，\texttt{case\_family}を受け取り，それに紐付く$v(A)$の範囲において，
実際に補題\ref{Lem2}の主張が成り立つかどうかを検証している．

\texttt{case\_family}の中身は以下のような形式である：
\begin{lstlisting}
  # Case 2.1
    {
        "hk": [(0, 0), (1, 1)],
...
\end{lstlisting}
この入力を受け取ったとき，\texttt{verify\_case\_family\_v\_cover}函数は，
$(-1)^{h + k} = 1$の場合，つまり$(h, k) = (0, 0)$または$(1, 1)$のとき，
$1\le v(A) \le 1.98$なる$A \in \mathcal{B}_{h,k}$について，補題\ref{Lem2}の主張が成り立つか検証している．つまり，
$I(\sim 2| \sim 2) , I(\sim 3| \sim 1, 1) , I(\sim 2, 3| \sim 2, 3), I(\sim 1, 1, 3| \sim 3, 1), I(\sim 1| ~)$
が全て$A$の許容的後継であって，それらの和集合が連結である，または，
$I(\sim 2| \sim 2) , I(\sim 3| \sim 1, 2) , I(\sim 2| \sim 1), I(\sim 1| \sim 1)$
が全て$A$の許容的後継であって，それらの和集合が連結であることを検証している．

具体的には，\texttt{cases}の要素の中の\texttt{intervals}の要素に含まれる
それぞれの要素に対応する後継が許容的であるのに充分な$v(A)$の範囲，さらに
となりあう二つの要素について，それらが繋がっているのに充分な$v(A)$の範囲を計算する．
そして\texttt{case\_family}の\texttt{v\_range}で指定された$v(A)$の範囲において，
どれかの\texttt{cases}においてそれらの条件がすべて成り立つかどうかを検証している．
...
\[
  \frac{5}{17}\min_{c^-, c^+ \in [0.2,0.9]}|H(r_1,r_2,s_1,s_2; c^+, c^-)|
  \le v(A)
  \le \frac{17}{5} \max_{c^-, c^+ \in [0.2,0.9]} |H(r_1,r_2,s_1,s_2; c^+, c^-)|
\]
であれば，充分である．

\texttt{compute\_H\_extrema}函数は与えられた$r_1,r_2,s_1,s_2$について，
$c^-, c^+$を指定の範囲で動かしたときの$H$の最大値と最小値を計算する．
この函数は$G$の最大値と最小値を計算する\texttt{\_g\_extrema\_on\_interval}函数を用いているが，
$G' = 0$が解ける形なので，$G' = 0$の点と端点での$G$の値の最大値と最小値を計算するという通常の方法で求まる．

\texttt{\_hilfsatz4\_successor\_v\_bounds} 函数が\texttt{compute\_H\_extrema}函数を用いて
一つの後継が許容的であるための$v(A)$の範囲を求め，，
\texttt{compute\_hilfsatz4\_common\_v\_bounds}函数がそれらの結果をまとめて，
\texttt{cases}の中の一つの場合についてそれらが全て許容的であるための$v(A)$の範囲を求める．
...
は
\[
  (-1)^{h} (v(A) - (-1)^{k}H(s_1, s_2, r_2, r_1; c^+ , c^-)) (r_2 - r_1) \ge 0 
\]
と同値であり，$(-1)^{h + k} = -1$のとき，
\begin{align*}
  [0; a_1,\cdots , a_{k} + r_1] +  [0; a_{-1},\cdots , a_{-h} + s_1]
...
\[
v(A) \le (-1)^k H(s_1, s_2, r_2, r_1; c^+ , c^-)
\]
なることが必要かつ充分な条件であり，
$(-1)^{h + k} = -1$で$(-1)^h(r_2 - r_1) > 0$の場合は，
\[
...
\texttt{\_condition\_iii\_v\_bounds\_from\_H\_extrema}では
\texttt{compute\_H\_extrema}によって計算した$H$の最大値と最小値をもとに，上記の計算を行い，
$v(A)$についての充分条件を求めている．
\end{comment}

\subsection{\texttt{verify\_case\_family\_v\_cover}による検証}
上記の計算を用いて，\texttt{verify\_case\_family\_v\_cover}によって全体の検証を行っている．

\texttt{case\_family}の\texttt{v\_range}で指定された範囲に$v(A)$が含まれる任意の$A \in \mathcal{B}_{h,k}$について，
どれかの\texttt{cases}の中の要素がすべて許容的後継で，かつ連結であることを示したい．
そのために，\texttt{cases}に含まれる一つの\texttt{case}について，次のような計算を行う：

まず，前節で求めたように$s_1, r_1, s_2, r_2$を求めたいのだが，$A$が不定であるがゆえに，一つに定まるとは限らない．
そのため，\texttt{select\_successor\_endpoint\_pairs}函数により，指定の$v(A)$の範囲でありうる組合せを全て列挙する．

\texttt{select\_successor\_endpoint\_pairs}函数は，例えば$s_1$として\verb|test|

\texttt{\_select\_pairings\_for\_v\_range}


\texttt{compute\_hilfsatz4\_common\_v\_bounds}函数によって，
含まれている区間が全て許容的であるための$v(A)$の範囲を求める．
