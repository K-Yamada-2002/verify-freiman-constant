...
\begin{lstlisting}[language=python,breaklines=true]
  ...
  case 0: internal connectivity check on (1, 1.98000000000000);
  compared=12, passed=12, all=True
    ...
    assigned=(1, 1.26723050245227); nonempty=True; admissibility covers assigned=True
  ...
  case 1: internal connectivity check on (1.25723050245227, 1.98000000000000);
  compared=6, passed=6, all=True
    ...
    static nonempty=True
    assigned=(1.25723050245227, 1.98000000000000); nonempty=True; admissibility covers assigned=True
  ...
  family summary: internal=True, assigned_nonempty=True, admissibility=True, connections=True, endpoints=True, all=True
\end{lstlisting}
...
`connectivity`（内部の上下界比較）、`assigned`（割当範囲が非空）、
`admissibility`（割当範囲の許容性）、`connect`（隣接ケースの接続）、
...
`print_case_family_verification` の計算は維持し、詳細出力の名称を許容性・連結性に揃えています。
