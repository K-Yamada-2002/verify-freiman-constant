...
まず$-h \le i \le k$における$\lambda_i$の値としてありうる最大値を\verb|max_value_of_lambda|を使って算出し
...
したがって，上限を考える際には，$-h \le i \le k$における$\lambda_i$の値の最大値に，$\sqrt{21}$を候補\verb|max_values|に追加するだけでよい．
...
\end{lstlisting}
この入力を受け取ったとき，\verb|verify_case_family_v_cover|函数は，
...
\verb|_hilfsatz4_successor_v_bounds| 函数が\verb|compute_H_extrema|函数を用いて
一つの後継が許容的であるための$v(A)$の範囲を求め，，
\verb|compute_hilfsatz4_common_v_bounds|函数がそれらの結果をまとめて，
...
\verb|_condition_iii_v_bounds_from_H_extrema|では
...
\verb|compute_condition_iii_v_bounds| 函数により，
...
\verb|check_condition_iii_bounds_compatible| 函数は，
...
\item 次に，\verb|compute_hilfsatz4_common_v_bounds|函数によって，
\verb|case|に含まれている後継が全て許容的であるために充分な$v(A)$の範囲を求める．

\item \verb|compute_condition_iii_v_bounds|と\verb|compute_hilfsatz4_common_v_bounds|
...
探索する$v(A)$の範囲を，前の\verb|case|で求めた範囲の上限（もしあれば）を下限とし，\verb|v_range|の上限を上限とする
区間に狭めて計算を行う．

さらに，上記の計算に加えて，次のような計算を行う：
\begin{itemize}
  \item \verb|check_condition_iii_transition|により，
  ...
  任意の$c^-, c^+ \in [0.2, 0.9]$について，前者が後者よりも小さいことを確かめる．このための$H$の値を比較にも，それには補題\ref{Hilfsatz3}を用いる．
\end{itemize}
...
具体的には，一番左の点線は(i)について計算をしたとき得られる下限の上限，
その次の点線は(i)について計算をしたとき得られる上限の下限，
...
（ただし，図では描かれていないが，(i)での許容性に関する上限が，(ii)についての計算で得られる下限を上回ることはありうる）

\subsection{検証の妥当性}
上記の計算結果を出力すると，以下のような結果を得る（見やすさのためにいくつか改行を入れた）．
\begin{lstlisting}[language=python]
  case family 2.1: (h, k)=(0, 0), v(A) in (1, 1.98000000000000)
  case 0: internal iii check on (1, 1.98000000000000); 
  compared=12, passed=12, all=True

    Hilfsatz 4=(0.910356373754828, 3.10088499093588); 
    iii=(0.713963480223948, 1.26723050245227); 
    intersection=(0.910356373754828, 1.26723050245227); 
    checked intersection=(1, 1.26723050245227); 
    static nonempty=True
    assigned=(1, 1.26723050245227); nonempty=True; H4 covers assigned=True
  
  case 0 -> case 1: previous_upper=1.26723050245227, effective_upper=1.26723050245227, 
  checked=(1.25723050245227, 1.98000000000000); compared=6, passed=6, all=True

  case 1: internal iii check on (1.25723050245227, 1.98000000000000); 
  compared=6, passed=6, all=True
    Hilfsatz 4=(0.808428126706645, 1.98750784542276); 
    iii=(1.44759026701375, 1.99635347193077); 
    intersection=(1.44759026701375, 1.98750784542276); 
    checked intersection=(1.44759026701375, 1.98000000000000); 
    static nonempty=True
    assigned=(1.25723050245227, 1.98000000000000); nonempty=True; H4 covers assigned=True

  endpoint coverage after Hilfsatz 4: 
  first_lower=0.910356373754828 <= 1: True; 
  last_upper=1.98750784542276 >= 1.98000000000000: True; 
  both=True
  
  family summary: internal=True, assigned_nonempty=True, Hilfsatz4=True, connections=True, endpoints=True, all=True
\end{lstlisting}

\begin{comment}
## `schecker_proof_2.ipynb`: 検証ログの読み方

上から実行すると、末尾にケース群 2.1–2.6 の要約表を一度だけ表示します。
`iii`（内部の上下界比較）、`assigned`（割当範囲が非空）、
`H4`（割当範囲の許容性）、`connect`（隣接ケースの接続）、
`ends`（両端の被覆）、`all`（以上の全チェック）を確認できます。
これは現行関数の数値的な判定であり、各群の `hk` の先頭の組について計算しています。

詳細は再計算せず、保存された `case_family_verification_results` から表示できます。

```python
show_verification_details(1)                          # ケース群 2.1：解説 PDF のログ形式
show_verification_details(1, case_index=0)            # 群内の case 0 の範囲と判定
show_verification_details(1, case_index=0, raw=True)  # 端点と H の比較まで表示
```

群番号は 1–6、群内のケース番号は 0 始まりです。通常の Run All では詳細を出力しません。
`doc/schecker.tex` が参照する `verify_case_family_v_cover` と
`print_case_family_verification` の計算・詳細出力は維持しています。

[整理前の保存用ノート](archive/schecker_proof_2_before_log_cleanup.ipynb) には、
整理直前のコード・実行番号・出力をそのまま保存しています。
旧診断、固定範囲での接続実験、後続探索、具体的な T 区間の実験はそちらを参照してください。
保存ログには入力や実行順が異なる結果が混在しているため、現行の検証結果として引用する際は
メインノートを再実行してください。保存用ノートを実行する場合は別カーネルを使います。
\end{comment}
